...
def fetch_all_integrated_news():
    ...
    # 2차 유사도 클러스터링 (대표 1건 + 서브 유사 기사 그룹화)
    clusters = []
    visited = set()
    ...
    for i in range(len(rows)):
        ...
        for j in range(i + 1, len(rows)):
            ...
            if main_art["카테고리"] == comp_art["카테고리"]:
                ...
                if sim >= 0.38:  # 38% 이상 유사하면 하위 그룹으로 묶음
...
st.write(f"⚡ 최근 36시간 수집 완료: 최신 대표 그룹 **{len(clusters)}개** (총 {total_articles_count}건) | 🧠 AI 학습 데이터 축적: **{history_count}건**")

if clusters:
    if st.button("🎯 중요 대표 기사 자동 선별하기 (카테고리별 상위 대표기사 자동 체크)", type="primary"):
        updated_clusters = []
        for c in clusters:
            c_copy = c.copy()
        ...
        for cat in CATEGORIES_LIST:
            ...
            for c in cat_cls[:5]:  # 상위 5개 대표 기사 체크
    ...
    for i, cat in enumerate(CATEGORIES_LIST):
        with tabs[i]:
            ...
            if cat_clusters:
                ...
                # 2. 하단에 서브 유사 기사 아코디언 세션 배치
                sim_exist_clusters = [c for c in cat_clusters if c["similars"]]
                if sim_exist_clusters:
                    st.markdown("---")
                    with st.expander(f"📁 {cat} 관련 유사/보도자료 기사 그룹 ({len(sim_exist_clusters)}개 대표기사에 유사기사 존재)", expanded=False):
    ...
    st.subheader(f"✅ 현재 총 **{selected_count}건**의 기사(대표기사 + 서브 유사기사)가 선택되었습니다.")
    ...
    if st.button("🚀 선택한 기사로 뉴스레터 생성하기"):
        if selected_count > 0:
            ...
            st.warning("선택된 기사가 없습니다. 기사를 체크해 주세요.")
